# From 2022 onwards, the MTA has updated the publication of turnstile data.
...
# List of months in the quarters
quarters <- list(
  Q1 = 1:3, 
  Q2 = 4:6, 
  Q3 = 7:9, 
...
## Part 1: Get and export as rds data from all 2023 quarters
...
for (quarter in names(quarters)) {
  ...
  for (month in quarters[[quarter]]) {
    print(month) # Since it takes a long time to fetch the data, this helps knowing how long it will still take
    
    # Start and end dates of the month
    start_date <- as.Date(sprintf("2023-%02d-01", month))
    end_date <- as.Date(ifelse(month == 12, "2024-01-01", sprintf("2023-%02d-01", month + 1)))
